...
class Resultado:
    ...
    # - Excepciones: lista de booleanos que indica donde se produjo un error en la segmentación
...
def AnalizarQR(img):
    ...
    try:
        # cadena = qreader.decode(image=img)
    ...
    if eco:print("QR analizado exitosamente: ",cadena)   
    return cadena
...
def ValidarImagen(fuente):
    ...
    try:
        ...
        return (ErrorDeProcesamiento(res, f"Imagen no encontrada.") ,None)
    except:
    ...
    if res_img is None:
        return (ErrorDeProcesamiento(res, f"Imagen corrupta, no válida o no encontrada."),None)
...
def ImprimirImagen(img, nombre):
    #rescalar la imagen para que se pueda mostrar (la dimensión máxima será 700px)
...
def ReorientarImagen(res,img_original):
    ...
    img = cv2.Canny(img, 3, 20)
    # ImprimirImagen(img,"Canny")
    ...
    #Detección de contornos
    contornos, jerarquia = cv2.findContours(img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_TC89_KCOS)
    ...
    contornos = sorted(contornos, key=lambda x: cv2.arcLength(x,True), reverse=True)[:5]
    ...
    for i,c in enumerate(contornos):
        ...
        largo=cv2.arcLength(c,True)
        #aproximar el contorno a un polígono con 4 vértices (el coeficiente de arcLength es un parámetro que se obtuvo tanteando)
        aproximacion=cv2.approxPolyDP(c, 0.015*cv2.arcLength(c, True), True)
        
        #si el contorno tiene menos de 4 vértices, se descarta (no es posible tratarlo como un rectángulo)
        if len(aproximacion)<4: continue

        #en ocasiones el rectángulo es descrito con más de 4 vértices, por lo que se tratará de identificar los 4 vértices más extremos
        if len(aproximacion)>4:
            #detectamos el punto más arriba
            max_y=max(aproximacion,key=lambda x: x[0][1])
            #detectamos el punto más abajo
            min_y=min(aproximacion,key=lambda x: x[0][1])
            #detectamos el punto más a la izquierda
            min_x=min(aproximacion,key=lambda x: x[0][0])
            #detectamos el punto más a la derecha
            max_x=max(aproximacion,key=lambda x: x[0][0])

            aproximacion=[[min_x],[max_y],[max_x],[min_y]]
            aproximacion=np.array(aproximacion)

        #si el área es mayor que el área máxima y el contorno tiene 4 vértices, se guarda el contorno
        if largo>largoMax:
            largoMax=largo
            #se guardan los vértices extremos en la variable contorno
    ...
    img=cv2.Canny(cv2.bilateralFilter(img,20,30,30),5,30)
    
    # ImprimirImagen(img,"Canny-Bilateral-Gaussiano")
    ...
    contorno=np.array([])
    longitudMax=0
    imagenContorno=img_original.copy()
    for c in contornos:
        longitud=cv2.arcLength(c,True)
        aproximacion=cv2.approxPolyDP(c,0.015*cv2.arcLength(c,True),True)
        if longitud>longitudMax:
            longitudMax=longitud
            contorno=aproximacion
        # dibujar los contornos en la imagen
        # cv2.drawContours(imagenContorno, [c], -1, (255, 255, 255), 10)

    
    #imprimir línea guía
    # cv2.drawContours(imagenContorno, [contorno], -1, (0, 255, 0), 20)
    # ImprimirImagen(imagenContorno,"Linea guia")
    ...
    #con base a la posición de los puntos, detectamos la rotación de la imagen

    p1,p2=(puntos[0],puntos[1])

    # #imprimir los puntos
    # imagenContorno=img_original.copy()
    # #este punto será de color rojo
    # cv2.circle(imagenContorno, tuple(p1),10, (0, 255, 0), -1)
    # #este punto será de color verde
    # cv2.circle(imagenContorno, tuple(p2),10, (0, 0, 255), -1)
    # ImprimirImagen(imagenContorno,"Puntos guía")
    ...
    # ImprimirImagen(img_original,"Final")
...
def SegmentarImagen(res,img,dimensionEntradaIA,contrastar=False,normalizar=False):
    ...
    for i in range(cantidadFilas):
        for j in range(cantidadColumnas):
            ...
            recorte_aux=cv2.Canny(
                cv2.bilateralFilter(
                        cv2.GaussianBlur(recorte_aux,(5,5),0)
                        ,20,30,30
                ),10,20
            )
    ...
    for i in range(len(qr_str)):
        ...
        res.excepciones.append(recortes[i][1])
...
def Preprocesar(fuente,dimensionEntradaIA=32,normalizar=False,contrastar=False,eco_=True):
    ...
    try:
        ...
        res=ErrorDeProcesamiento(res,"Error de procesamiento desconocido, revise la imagen")
        # imprimir el error
        print(e)
        print("Preprocesamiento de ",fuente," fallido.","Razón: ",res.notaDeError)
    return res
# pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu117
# print(Preprocesar("C:/Users/caelo/OneDrive/Documents/PALALABack/PALALABack/humanoma.jpg",dimensionEntradaIA=32*2,normalizar=True,eco_=True).preprocesadoExitoso)
